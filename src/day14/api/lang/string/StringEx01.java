...
public class StringEx01 {
	...
	public static void main(String[] args) {
		...
		String str = "홍길동 입니다~! 안녕하세요?"; // 원래는 클래스
		...
		// substring() -
		System.out.println(str.substring(5)); // 5 미만은 절삭, 5 이상 문자열 추출
		...
		System.out.println(str.indexOf("헉")); // 없으면 -1 반환
		
		if(str.indexOf("헉") != -1) { // 헉 이라는 문자가 있다는 조건
		...
		// 양측 공백 제거
		str = "    abcaaaaaaaaaa 123   ";
		System.out.println(str.trim());

		// 모든 공백 제거 (문자열 변경)
		String s = str.replace("a", "헉"); // 대상문자, 변경할 문자
		System.out.println(s); // 원본 문자에는 반영되지 않음
		...
		// 문자열 대소 비교 - 사전등재순을 기준으로 
		System.out.println("홍길동".compareTo("이순신")); // 뒤의 문자가 사전적으로 앞에 있음
		System.out.println("홍길동".compareTo("홍길자")); // 음수: 뒤의 문자가 사전적으로 더 뒤에 있음
		System.out.println("홍길동".compareTo("홍길동")); // 0이 나오면 같은 문자
